<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\MaterialWarehouseOutOrder;

/**
 * SearchMaterialWarehouseOutOrder represents the model behind the search form about `app\models\MaterialWarehouseOutOrder`.
 */
class SearchMaterialWarehouseOutOrder extends MaterialWarehouseOutOrder
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['material_out_orderid', 'employee_id', 'department_id', 'material_out_ordertime', 'material_out_orderstate'], 'integer'],
            [['material_out_orderremark'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = MaterialWarehouseOutOrder::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'material_out_orderid' => $this->material_out_orderid,
            'employee_id' => $this->employee_id,
            'department_id' => $this->department_id,
            'material_out_ordertime' => $this->material_out_ordertime,
            'material_out_orderstate' => $this->material_out_orderstate,
        ]);

        $query->andFilterWhere(['like', 'material_out_orderremark', $this->material_out_orderremark]);

        return $dataProvider;
    }
}
